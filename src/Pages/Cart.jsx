...
import { decremntQuant, emptyCart, incrementQuant, removeCart } from '../Redux/Slice/cartSlice'
...
function Cart() {
  ...
  const handleDecrementQty=(Pcart)=>{
    if(Pcart.quantity>1 ){
      // console.log(Pcart.quantity);
      dispatch(decremntQuant(Pcart))
    }else{
      dispatch(removeCart(Pcart.id))
            ...
            <tbody>
              { cart.map((item,index)=>(
                ...
                <td><div className='d-flex'>
                  <button onClick={()=>handleDecrementQty(item)}  className='btn fw-bolder'>-</button>
                  <input style={{width:'50px'}} className='form-control' type="text" value={item.quantity} readOnly/> {/* we can given as form or just {item.quantity}  */}
                  <button onClick={()=>dispatch(incrementQuant(item))} className='btn fw-bolder'>+</button>
                  </div></td> 
